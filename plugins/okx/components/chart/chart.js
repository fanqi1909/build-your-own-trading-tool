...
export default class ChartComponent extends Component {
  ...
  async init() {
    ...
    this._currentBar  = ctx.bar || '15m';
    this._currentInst = ctx.instrument || 'BTC-USDT-SWAP';
    this._chart       = null;
    this._series      = null;
    ...
    this.el.innerHTML = `
      <div class="chart-card">
        <div class="chart-card__header">
          <div class="chart-card__title" id="chart-title">${this._formatInst(this._currentInst)} Candles</div>
          <div class="chart-card__bar-select" id="chart-bar-select">
            <button class="chart-card__bar-btn" data-bar="3m">3m</button>
            <button class="chart-card__bar-btn" data-bar="15m">15m</button>
  ...
  _syncHeader() {
    const title = this.el.querySelector('#chart-title');
    if (title) title.textContent = `${this._formatInst(this._currentInst)} Candles`;
  }
  ...
  _createChart() {
    ...
    this._chart = LightweightCharts.createChart(body, {
      layout: { background: { color: '#0c1220' }, textColor: '#64748b' },
      grid: {
    ...
    this._series = this._chart.addSeries(LightweightCharts.CandlestickSeries, {
      upColor: '#22c55e', downColor: '#f43f5e', borderUpColor: '#22c55e', borderDownColor: '#f43f5e', wickUpColor: '#22c55e', wickDownColor: '#f43f5e',
    });
  ...
  update({ data, inst, bar }) {
    ...
    if (inst && inst !== this._currentInst) return;
    if (bar && bar !== this._currentBar) return;
  ...
  _injectStyles() {
    ...
    s.id = id;
    s.textContent = `
      .chart-card { background: var(--bg-card, #0c1220); border: 1px solid var(--border, rgba(255,255,255,0.07)); border-radius: var(--radius, 6px); overflow: hidden; display: flex; flex-direction: column; }
      .chart-card__header { padding: 14px 20px; border-bottom: 1px solid var(--border, rgba(255,255,255,0.07)); display: flex; align-items: center; justify-content: space-between; gap: 10px; flex-wrap: wrap; }
      .chart-card__title { font-size: 13px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: var(--text, #e2e8f0); }
      .chart-card__bar-select { display: flex; gap: 4px; }
      .chart-card__bar-btn { padding: 3px 10px; font-size: 10px; font-family: var(--font-mono, 'SF Mono', monospace); background: none; border: 1px solid var(--border, rgba(255,255,255,0.07)); border-radius: 5px; color: var(--text-muted, #64748b); cursor: pointer; transition: .15s; }
      .chart-card__bar-btn:hover { border-color: rgba(0,229,255,0.3); color: #00e5ff; }
      .chart-card__bar-btn--active { border-color: #00e5ff; color: #00e5ff; background: rgba(0,229,255,0.08); }
      .chart-card__body { flex: 1; min-height: 460px; }
      .chart-card__footer { padding: 10px 20px; display: flex; justify-content: space-between; font-size: 10px; color: var(--text-muted, #64748b); border-top: 1px solid var(--border, rgba(255,255,255,0.07)); }
    `;
    document.head.appendChild(s);
  }
}
