...
export class ChatPanel {
  ...
  init() {
    this._suggestionEl = null;   // at most one active suggestion card
    this._injectStyles();
    this.el.innerHTML = `
      <div class="chat">
    ...
    this._addMessage('assistant',
      'Hello! I can help you customize your dashboard.\n\nTry: "Show me the chart" or "What can I do?"'
    );
  }
  ...
  _send() {
    ...
    const enabledIds = window.app?.layout?.list() || [];
    this.bus.emit('ws:send', { action: 'chat', message, enabledIds });
  }
  ...
  _injectStyles() {
    ...
    s.textContent = `
      ...
      .chat__reset-btn {
        ...
        padding: 3px 9px; font-size: 14px; transition: color .15s, border-color .15s;
      }
      ...
      .chat__msg--user {
        align-self: flex-end;
        background: rgba(33,150,243,0.15); border: 1px solid rgba(33,150,243,0.25);
        color: var(--text);
      }
      .chat__msg--assistant {
      ...
      .chat__msg--streaming .chat__msg-text::after {
        content: '▋'; animation: chat-blink 1s step-start infinite;
        color: var(--blue);
      }
      ...
      .chat__send-btn:disabled { opacity: 0.4; cursor: default; }
      .chat__send-btn:not(:disabled):hover { opacity: 0.85; }

      .chat__suggestion {
        background: rgba(77,158,255,0.07); border: 1px solid rgba(77,158,255,0.2);
